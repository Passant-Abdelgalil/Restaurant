...
GUI::GUI()
{
	...
	DrawingColors[TYPE_DESSERT] = BLACK;
	DrawingColors[TYPE_CHINA] = GREEN;
...
void GUI::PrintMessage(string msg, int nVLine, char writeHorizontal) const	//Prints a message on status bar
{

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	if (writeHorizontal == 'L') // You may need to change these coordinates later 
		pWind->DrawString(10, WindHeight - (int)((1.25 - (0.2 * (nVLine - 1))) * StatusBarHeight / 1.5), msg);
	else if (writeHorizontal == 'R')
		pWind->DrawString(WindWidth / 2, WindHeight - (int)((1.25 - (0.2 * (nVLine - 1))) * StatusBarHeight / 1.5), msg);
}
//////////////////////////////////////////////////////////////////////////////////////////
...
bool GUI::isDrawen(Order* pOrd)
{
	for (int i = 0; i < maxItemCnt; i++)
	{
		if (DrawingList[i]->ID == pOrd->GetID())
			return true;
	}
	return false;
}
...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	case WAIT:
		...
		break;
	case SRV:
		reg = SRV_REG;	//region of serving orders
		break;
	case DONE:
		reg = DONE_REG;	//region of finishing orders
		break;
	}
...
PROG_MODE	GUI::getGUIMode() const
{
	...
	do
	{
		PrintMessage("Please select GUI mode:  (1)Interactive,  (2)StepByStep,  (3)Silent........ ",1,'L');
		string S = GetString();
